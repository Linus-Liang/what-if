...
class StudentGradesViewModel {
    ...
    updateAssignmentEntry(newGrade) {
        // this = an AssignmentEntry from this.assignmentView
        const grade = this.grades.find(g => g.assignmentId === this.assignmentId);
        ...
        const currentGradedAssignment = this.gradedAssignments.find(ga => ga.assignmentId === this.assignmentId);
    ...
    updateSummery() {
        const gradedCategories = this._gradingService.averageStudents([this.student], // [this.student()] is done since the GradedService grades multiple students
            this.categories, this.gradedAssignments)[0].categories;
    ...
    init(student, categories, assignments, grades, scoreCodes) {
        ...
        const assignmentView = assignments.map(assignment => {
            ...
            return new AssignmentEntry({ ...assignmentCategory, ...assignment, categoryName: assignmentCategory.name }, gradedAssignment, this.updateAssignmentEntry);
        });
